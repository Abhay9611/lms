...
const AddContentModal = ({ isOpen, onClose }: AddContentModalProps) => {
  ...
  useEffect(() => {
    const fetchGrades = async () => {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/grades`);
  ...
  useEffect(() => {
    if (formData.topic) {
      const fetchContents = async () => {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/contents`);
      ...
      const fetchMaterials = async () => {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/teaching-guides`);
  ...
  const handleAddVideo = async () => {
    ...
    try {
      ...
      await axios.post(`${import.meta.env.VITE_API_URL}/contents`, newVideo);
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/teaching-guides/upload`, fileFormData, {
        headers: {
  ...
  const handleAddMaterial = async () => {
    ...
    try {
      ...
      await axios.post(`${import.meta.env.VITE_API_URL}/teaching-guides`, newMaterial);
  ...
  const handleDeleteVideo = async (videoId) => {
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL}/contents/${videoId}`);
  ...
  const handleDeleteMaterial = async (materialId) => {
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL}/teaching-guides/${materialId}`);
