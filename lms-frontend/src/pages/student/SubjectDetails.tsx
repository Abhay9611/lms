...
const SubjectDetails = () => {
  ...
  useEffect(() => {
    const fetchSubjectDetails = async () => {
      ...
      try {
        ...
        const subjectRes = await axios.get(`${import.meta.env.VITE_API_URL}/subjects/${id}`);
        ...
        const topicsRes = await axios.get(`${import.meta.env.VITE_API_URL}/topics`);
        const contentsRes = await axios.get(`${import.meta.env.VITE_API_URL}/contents`);
        const materialsRes = await axios.get(`${import.meta.env.VITE_API_URL}/teaching-guides`);
        const quizzesRes = await axios.get(`${import.meta.env.VITE_API_URL}/quizzes`);
  ...
  const handleQuizComplete = (score: number, total: number) => {
    ...
    const updateProgress = async () => {
      const res = await axios.post(`${import.meta.env.VITE_API_URL}/topics/progress/${currentTopic.id}`, {
        completedItem: "quiz"
      });
    }
    updateProgress();
                          ...
                          currentTopic.materials.map((material) => (
                            <div key={material.url} className="flex items-center justify-between p-4 border-b">
                              ...
                              <Button
                                variant="outline"
                                size="icon"
                                onClick={() => window.open(`${import.meta.env.VITE_API_URL}/uploads/${material.url}`, '_blank')}
                                className="rounded-full"
                              >
