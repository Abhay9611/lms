      ...
      try {
        ...
        const filteredTopics: Topic[] = topicsRes.data.filter((topic: any) => topic.subjectId === id).map((topic: any) => {
          ...
          const quizzes = quizzesRes.data.filter((quiz: any) => quiz.topicId === topic.id).map((quiz: any) => {
            ...
            const correctOptionIndex = [quiz.option1_iscorrect, quiz.option2_iscorrect, quiz.option3_iscorrect, quiz.option4_iscorrect].findIndex(isCorrect => isCorrect);
            const correctAnswer = options[correctOptionIndex];
            const explanation = [quiz.option1_explanation, quiz.option2_explanation, quiz.option3_explanation, quiz.option4_explanation][correctOptionIndex];
            return {
              id: quiz.id,
                    ...
                    {subjectDetails.topics.map((topic) => (
                      <Button 
                        ...
                        className={`w-full justify-start text-left rounded-xl ${
                          topic.id === (selectedTopic?.id || subjectDetails.topics[0].id) 
