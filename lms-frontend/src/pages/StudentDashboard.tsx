  ...
  useEffect(() => {
    const fetchSubjects = async () => {
      ...
      try {
        ...
        const subjects = await axios.get(`${import.meta.env.VITE_API_URL}/subjects`);
        const gradeSubjects = subjects.data.filter((s: any) => s.gradeId === user.gradeId)

        const studentProgressRes = await axios.get(`${import.meta.env.VITE_API_URL}/progress`);
        const progressData = studentProgressRes.data.filter((progress: any) => user.id == progress.userId);

        const grade = await axios.get(`${import.meta.env.VITE_API_URL}/grades`);
